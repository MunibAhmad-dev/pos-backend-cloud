...
// ─── Customer Khata (custom debt ledger) ─────────────────────────────────────

/**
 * GET /api/customers/khata   [instanceAuth]
 * Query: customer_id (required)
 *
 * Returns all open khata entries for a customer with payment totals,
...
router.get('/khata', requireInstance, async (req: Request, res: Response) => {
  ...
  const [khataEvents, paymentEvents] = await Promise.all([
    prisma.syncEvent.findMany({
      where:   { instance_id: instanceId, entity_type: 'customer_khata' },
      orderBy: { id: 'asc' },
      select:  { operation: true, payload: true },
    }),
    prisma.syncEvent.findMany({
      where:   { instance_id: instanceId, entity_type: 'customer_khata_payment' },
      orderBy: { id: 'asc' },
      select:  { operation: true, payload: true },
    }),
  ]);
  ...
  const customerKhata = Array.from(khataMap.values()).filter(k => String(k.customer_id) === customerId);

  if (customerKhata.length === 0) {
    res.json({ success: true, data: [] });
    return;
  }

  // Accumulate payments per khata id (live events only — deletes subtract)
  const khataIds = new Set(customerKhata.map(k => String(k.id)));
  ...
  const data = customerKhata.map(k => {
...
/**
 * GET /api/customers/khata/:khataId/payments   [instanceAuth]
 *
...
/**
 * GET /api/customers/khata/summary   [instanceAuth]
 *
 * Returns per-customer totals: total_khata, total_khata_paid, khata_balance.
 * Useful for admin dashboards showing outstanding customer debts across the store.
 */
router.get('/khata/summary', requireInstance, async (req: Request, res: Response) => {
  ...
  // Group khata total_amount by customer_id
  const totalByCustomer = new Map<string, number>();
  for (const k of khataMap.values()) {
    const cid = String(k.customer_id ?? '');
    totalByCustomer.set(cid, (totalByCustomer.get(cid) ?? 0) + Number(k.total_amount ?? 0));
  }

  // Accumulate payments by khata_id → then map to customer_id
  const khataToCustomer = new Map<string, string>();
  for (const k of khataMap.values()) khataToCustomer.set(String(k.id), String(k.customer_id ?? ''));
  ...
  for (const ev of paymentEvents) {
    ...
    try { payload = JSON.parse(ev.payload); } catch { continue; }
    const kid = String(payload?.khata_id ?? '');
    const cid = khataToCustomer.get(kid);
    if (!cid) continue;
    const amt = Number(payload?.amount ?? 0);
    const prev = paidByCustomer.get(cid) ?? 0;
    paidByCustomer.set(cid, ev.operation === 'delete' ? prev - amt : prev + amt);
  }

  const data = Array.from(totalByCustomer.entries()).map(([customer_id, total_khata]) => {
    const total_paid = paidByCustomer.get(customer_id) ?? 0;
    return { customer_id, total_khata, total_khata_paid: total_paid, khata_balance: total_khata - total_paid };
  });
