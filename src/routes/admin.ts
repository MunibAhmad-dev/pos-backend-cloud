...
// ─── Shop Accounts (multi-device) ────────────────────────────────────────────

/** Generates a unique human-readable shop code: SHOP-XXXX1234 */
function generateShopCode(): string {
  const letters  = 'ABCDEFGHJKLMNPQRSTUVWXYZ';
  const digits   = '0123456789';
  const randStr  = (src: string, n: number) =>
    Array.from({ length: n }, () => src[Math.floor(Math.random() * src.length)]).join('');
  return `SHOP-${randStr(letters, 4)}${randStr(digits, 4)}`;
}

/**
 * POST /api/admin/shop-accounts
 * Body: { shop_name, username, password, owner_name?, owner_mobile?, max_devices?, notes? }
 */
router.post('/shop-accounts', requireAdmin, async (req: Request, res: Response) => {
  const { shop_name = '', username, password, owner_name = '', owner_mobile = '',
          max_devices = 5, notes = '' } = req.body as Record<string, any>;
  ...
  const existing = await prisma.shopAccount.findUnique({ where: { username: username.trim() } });
  if (existing) {
    res.status(409).json({ success: false, error: 'Username already taken' });
    return;
  }

  let shop_code = generateShopCode();
  // Ensure uniqueness (collision is astronomically rare but be safe)
  while (await prisma.shopAccount.findUnique({ where: { shop_code } })) {
    shop_code = generateShopCode();
  }

  const password_hash = await bcrypt.hash(password.trim(), 10);

  const account = await prisma.shopAccount.create({
    data: {
      shop_code, username: username.trim(), password_hash,
      shop_name: shop_name.trim(), owner_name: owner_name.trim(),
  ...
  res.status(201).json({
    success: true,
    data: {
      id: account.id, shop_code, username: account.username,
      shop_name: account.shop_name, owner_name: account.owner_name,
      owner_mobile: account.owner_mobile, max_devices: account.max_devices,
 ...
 * Returns all shop accounts with device counts and last-active device heartbeat.
 */
router.get('/shop-accounts', requireAdmin, async (_req: Request, res: Response) => {
  const accounts = await prisma.shopAccount.findMany({
    orderBy: { created_at: 'desc' },
    include: {
      devices: {
        select: { id: true, device_name: true, device_type: true, device_role: true,
                  last_heartbeat: true, is_active: true, registered_at: true },
      },
    },
  });

  const now = Date.now();
  const ONLINE_MS = 5 * 60_000; // consider online if heartbeat < 5 min ago
  ...
  res.json({
    ...
    data: accounts.map(a => ({
      ...
      owner_mobile: a.owner_mobile,
      max_devices:  a.max_devices,
      is_active:    a.is_active,
      notes:        a.notes,
      created_at:   a.created_at,
      device_count: a.devices.filter(d => d.is_active).length,
      online_count: a.devices.filter(d =>
        d.is_active && d.last_heartbeat &&
        now - new Date(d.last_heartbeat).getTime() < ONLINE_MS
      ).length,
      devices: a.devices.map(d => ({
        ...
        online: d.is_active && !!d.last_heartbeat &&
                now - new Date(d.last_heartbeat).getTime() < ONLINE_MS,
      })),
    })),
  });
...
/**
 * GET /api/admin/shop-accounts/:id
 */
router.get('/shop-accounts/:id', requireAdmin, async (req: Request, res: Response) => {
  const id = Number(req.params.id);
  const account = await prisma.shopAccount.findUnique({
    where: { id },
    include: { devices: { orderBy: { registered_at: 'asc' } } },
  });
  if (!account) {
    res.status(404).json({ success: false, error: 'Shop account not found' });
    return;
  }
  const now = Date.now();
...
/**
 * PATCH /api/admin/shop-accounts/:id
 * Update shop name, max_devices, password, active status, etc.
 */
router.patch('/shop-accounts/:id', requireAdmin, async (req: Request, res: Response) => {
  const id = Number(req.params.id);
  const { shop_name, owner_name, owner_mobile, max_devices, is_active, notes, password } =
    req.body as Record<string, any>;

  const data: Record<string, any> = {};
  if (shop_name    !== undefined) data.shop_name    = shop_name.trim();
  if (owner_name   !== undefined) data.owner_name   = owner_name.trim();
  if (owner_mobile !== undefined) data.owner_mobile = owner_mobile.trim();
  if (max_devices  !== undefined) data.max_devices  = Number(max_devices);
  if (is_active    !== undefined) data.is_active    = Boolean(is_active);
  if (notes        !== undefined) data.notes        = notes.trim();
  if (password?.trim())           data.password_hash = await bcrypt.hash(password.trim(), 10);

  try {
    const updated = await prisma.shopAccount.update({ where: { id }, data });
    res.json({ success: true, data: { ...updated, password_hash: undefined } });
  } catch {
    res.status(404).json({ success: false, error: 'Shop account not found' });
  }
});
...
/**
 * DELETE /api/admin/shop-accounts/:id/devices/:deviceId
 * Revoke (soft-delete) a specific device — it can no longer use heartbeat or shop-feed.
 */
router.delete('/shop-accounts/:id/devices/:deviceId', requireAdmin, async (req: Request, res: Response) => {
  const deviceId = Number(req.params.deviceId);
  try {
    await prisma.shopDevice.update({ where: { id: deviceId }, data: { is_active: false } });
...
/**
 * POST /api/admin/shop-accounts/:id/reset-password
 * Body: { new_password }
 */
router.post('/shop-accounts/:id/reset-password', requireAdmin, async (req: Request, res: Response) => {
  const id = Number(req.params.id);
  const { new_password } = req.body as { new_password: string };
  if (!new_password?.trim()) {
    res.status(400).json({ success: false, error: 'new_password is required' });
    return;
  }
  const password_hash = await bcrypt.hash(new_password.trim(), 10);
  try {
    await prisma.shopAccount.update({ where: { id }, data: { password_hash } });
    res.json({ success: true, message: 'Password updated' });
  } catch {
    res.status(404).json({ success: false, error: 'Shop account not found' });
  }
